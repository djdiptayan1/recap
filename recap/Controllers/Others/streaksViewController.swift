...
class StreaksViewController: UIViewController {
    ...
    // Initialize calendarData to track streaks for each day
    var calendarData: [Int: Bool] = [:]
    ...
    override func viewDidLoad() {
        ...
        setupHeaderView()
        setupCalendarView()
        updateCalendarData() // Update calendar with data
    }
    
    func updateCalendarData() {
        // Recalculate streaks for the new month
        calendarData.removeAll()  // Clear old data
        
        let numberOfDaysInMonth = daysInMonth()
        
        // Set up streaks for demonstration (this can be dynamic based on your data)
        for day in 1...numberOfDaysInMonth {
            // Example: Mark odd days as active for demo purposes
            calendarData[day] = day % 2 == 1
        }
        
        // Refresh collection view after updating calendar data
    ...
    @objc func handlePreviousMonth() {
        ...
        monthYearLabel.text = formattedMonthYear()
        updateCalendarData()  // Update data and reload calendar
    }
